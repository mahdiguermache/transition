    ...
    for (let i = 0; i < overlapArray.length; i++) {
        ...
        for (let j = 0; j < nbOverlapped; j++) {
            ...
            if (overlapArray[i].directions[j]) {
                const offsetLine = turf.lineOffset(JSON.parse(segment), 10 * sameDirectionOffset, { units: 'meters' });
                ...
                const offsetLine = turf.lineOffset(reverseLine, 10 * oppositeDirectionOffset, { units: 'meters' });
                replaceCoordinate(
