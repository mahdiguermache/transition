    ...
    for(let i = 0; i < overlapArray.length ; i++){
        ...
        for(let j=0; j < nbOverlapped ; j++){
            ...
            if(overlapArray[i].directions[j]){
                ...
                replaceCoordinate(JSON.stringify(reverseLine),JSON.stringify(offsetLine),overlapArray[i].crossingLines[j]);
...
const replaceCoordinate = (lineToReplace:string, offsetLine:string, lineId:number) => {
    ...
    let line = JSON.parse(getLineById(lineId));
    ...
    let lineIndex = getLineIndexById(lineId);
